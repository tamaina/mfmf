...
function _makeNode(name, children, props) {
    return children ? {
        name,
        children,
        props
...
function makeNode(name, props) {
    return _makeNode(name, null, props);
}
function makeNodeWithChildren(name, children, props) {
    return _makeNode(name, children, props);
}
function getTrailingPosition(x) {
    const brackets = [
        ['(', ')'],
        ['「', '」'],
    ];
...
const mfm = P.createLanguage({
    root: r => P.alt(r.big, r.bold, r.motion, r.url, r.link, r.mention, r.hashtag, r.emoji, r.blockCode, r.inlineCode, r.quote, r.math, r.search, r.title, r.center, r.text).atLeast(1),
    ...
    center: r => P.regexp(/<center>([\s\S]+?)<\/center>/, 1)
        .map(x => makeNodeWithChildren('center', P.alt(r.big, r.bold, r.motion, r.mention, r.hashtag, r.emoji, r.math, r.url, r.link, r.text).atLeast(1).tryParse(x))),
    //#endregion
    //#region Emoji
    ...
    link: r => P.seqObj(['silent', P.string('?').fallback(null).map(x => x != null)], P.string('['), ['text', P.regexp(/[^\n\[\]]+/)], P.string(']'), P.string('('), ['url', r.url], P.string(')'))
        .map((x) => {
        return makeNodeWithChildren('link', P.alt(r.big, r.bold, r.motion, r.emoji, r.text).atLeast(1).tryParse(x.text), {
            silent: x.silent,
            url: x.url.props.url
        });
    }),
    ...
    motion: r => P.alt(P.regexp(/\(\(\(([\s\S]+?)\)\)\)/, 1), P.regexp(/<motion>(.+?)<\/motion>/, 1))
        .map(x => makeNodeWithChildren('motion', P.alt(r.bold, r.mention, r.hashtag, r.emoji, r.url, r.link, r.math, r.text).atLeast(1).tryParse(x))),
    //#endregion
    //#region Quote
    ...
    //#endregion
    //#region Search
    search: r => newline.then(P((input, i) => {
        const text = input.substr(i);
    ...
    title: r => newline.then(P((input, i) => {
        ...
        const contents = P.alt(r.big, r.bold, r.motion, r.url, r.link, r.mention, r.hashtag, r.emoji, r.inlineCode, r.text).atLeast(1).tryParse(q);
